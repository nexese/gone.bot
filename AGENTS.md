...
- The reviewed, location-appropriate broker set is an active-session queue, not bulk authorization. Work through it one case at a time, with a separate exact-action preview and explicit approval before every disclosure or submission. Do not disclose sensitive identifiers merely to test whether a company has a file, and do not create a new consumer file or account solely to freeze it. If an official flow reports no matching file, record that limited observation and continue to the next queued profile unless the user chooses a documented next step.
